...
/**
    Defines a Factory class for allocating Locations of a Field in memory.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
@:nativeGen
class AllocatorLocation<T> implements Allocator<T> {
    ...
    public function recommendedArray(x : Int, y : Int, fill : Bool) : Pool<T> {
        if (x < 0) {
            return new PoolArray2D<T>(this);
        } else {
            return new PoolVector2D<T>(this, x, y);
        }
    }

    public function recommendedMap(x : Int, y : Int, fill : Bool) : Pool<T> {
        return new PoolMap2D<T>(this);
    }
