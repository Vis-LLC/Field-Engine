...
/**
    A Concrete Strategy for allocating parts of a Field in memory in a "lazy manner" and we want the object last indefinitely.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
@:nativeGen
class LazyPermanent<T, F, L, S> implements Manager<T, L, S> {
    public function new() { }

    public function preallocate(allocator : Allocator<T>, fField : FieldInterface<L, S>, x : Int, y : Int) : Pool<T> {
        return allocator.recommendedMap(x, y, true);
    }
