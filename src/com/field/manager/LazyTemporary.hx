...
/**
    A Concrete Strategy for allocating parts of a Field in memory in a "lazy manner" and we want the usage to be temporary.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
@:nativeGen
class LazyTemporary<T, F, L, S> implements Manager<T, L, S> {
    public function new() { }
