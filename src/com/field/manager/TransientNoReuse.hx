...
/**
    A Concrete Strategy for allocating parts of a Field in memory in a manner where the usage is not tracked in any meaningful way.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
@:nativeGen
class TransientNoReuse<T, F, L, S> implements Manager<T, L, S> {
    public function new() { }
    ...
    public function startWith(allocator : Allocator<T>, fField : FieldInterface<L, S>, pool : Pool<T>, discard : Pool<T>, x : Int, y : Int) : T {
        #if js
            return cast (cast allocator.allocate()).init(fField, x, y);
        #else
            return cast cast(allocator.allocate(), Usable<Dynamic, Dynamic>).init(fField, x, y);
        #end
    }
