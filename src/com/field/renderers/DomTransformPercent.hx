...
@:nativeGen
/**
    A Bridge for determining how a FieldView should render it's display using DOM (Document Object Model) transforms, with the information represented in Percentages.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
class DomTransformPercent extends RendererDomAbstract {
    public static var _instance : RendererInterface = new DomTransformPercent();
    ...
    public override function scroll(e : Element, x : Float, y : Float, dRectWidth : Float, dRectHeight : Float, dTileWidth : Float, dTileHeight : Float, f : Null<Void -> Void>, tempMode : Null<RendererMode>) : RendererInterface {
        if (x != 0 || y != 0) {
            ...
            var s : String = "translate3d(" + (x / dTileWidth) * 100.0 + "%" + "," + (y / dTileHeight) * 100.0 + "%" + ",0px)";
    ...
    public override function getX(e : Element, dRectWidth : Float, dTileWidth : Float, scale : Float) : Float {
        ...
        if (_alwaysUseCache) {
            ...
            if (x == null) {
                ...
                x = parseTransformForX(transform, 1, 100, null);
            }
            return x;
        } else {
            transform = getStyleTransform(e);
            if (transform == cast getXCache(e)) {
                ...
                return parseTransformForX(transform, 1, 100, null);
            }
        }
    }
    ...
    public override function getY(e : Element, dRectHeight : Float, dTileHeight : Float, scale : Float) : Float {
        ...
        if (_alwaysUseCache) {
            ...
            if (y == null) {
                transform = getStyleTransform(e);
                y = parseTransformForY(transform, 1, 100, null);
            }
            return y;
        } else {
            transform = getStyleTransform(e);
            if (transform == cast getYCache(e)) {
                ...
                return parseTransformForY(transform, 1, 100, null);
            }
        }
    }
