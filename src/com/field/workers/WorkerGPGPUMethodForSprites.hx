...
@:nativeGen
/**
    A compiled routine that can be run.  This runs over just the Sprites in a Field.
    These functions are not meant to used externally, only internally to the FieldEngine library.
**/
class WorkerGPGPUMethodForSprites extends WorkerGPGPUMethodAbstract {
    public function new() {
        super();
    }
    // TODO - More?
    public override function execute(field : FieldInterface<Any, Any>, initParams : Null<NativeVector<Int>>, methodParams : Null<NativeVector<Int>>, finalParams : Null<NativeVector<Int>>, callback : WorkerGPGPUResult -> Void) : Void {
        #if js
            var f : NativeVector<Int> -> Int -> Int -> NativeVector<Int> -> Any = cast _kernel;
            var sys : FieldSystemInterface<Any, Any> = cast field;
            callback(cast f(sys.getSpriteMemory(), sys.getSpriteAttributeCount(), sys.getMaximumNumberOfSprites(), methodParams));
        #else
            // TODO
        #end
    }
